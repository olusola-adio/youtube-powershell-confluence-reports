...
try {
    ...
    & $PSScriptRoot/PublishConfluenceReport.ps1 -BaseURI $($BaseURI) -confluenceApiUsername $($ConfluenceApiUsername) -ConfluenceApiTokenPass $($ConfluenceApiTokenPass) -ConfluencePageId $($ConfluenceInventoryPageId) -ReportName $($ReportName) -ConfluenceTable $($ConfluenceTable)
}
catch {
    throw $_
}